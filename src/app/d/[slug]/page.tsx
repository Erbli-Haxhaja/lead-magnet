import { db } from "@/db";
import { documents, documentViews, posts } from "@/db/schema";
...
export default async function LeadMagnetPage({ params, searchParams }: PageProps) {
  const { slug } = await params;
  ...
  // Validate ref param — must be a real post for this document
  let postId: string | null = null;
  if (ref) {
    const [post] = await db
      .select({ id: posts.id })
      .from(posts)
      .where(and(eq(posts.id, ref), eq(posts.documentId, doc.id)))
      .limit(1);
            ...
            <div className="bg-htd-card rounded-2xl overflow-hidden">
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-2/3 p-8 md:p-10">
                  ...
                  {/* Lead capture flow */}
                  <LeadCaptureFlow slug={doc.slug} title={doc.title} postId={postId} />
                </div>
              </div>
            </div>
          </div>
